 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum and max can summax numberz") {
    assert(sum(List(max(List(3, 2)), max(List(9, 2)))) === 12)
  }

  test("practice exception test") {
    intercept[IllegalArgumentException] {
      intNotZero(0)
    }
  }
  ...
  test("summore summax") {
    assert(max(List(sum(List(3, 2, 1)), sum(List(9, 8, 7)))) === 24)
  }
